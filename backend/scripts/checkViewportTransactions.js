...
const checkViewportTransactions = async () => {
  try {
    console.log('🔍 Checking transactions to "Viewport" merchant...');
    ...
    // Find all transactions to "Viewport"
    const viewportTxns = await Transaction.find({
      userId: user._id,
      merchant: { $regex: new RegExp('Viewport', 'i') },
    }).sort({ transactionDate: -1 });
    
    console.log(`\n📊 Found ${viewportTxns.length} transactions to "Viewport":`);
    
    if (viewportTxns.length === 0) {
      console.log('   No transactions found for "Viewport" merchant');
      return;
    }
    
    viewportTxns.forEach((txn, index) => {
    ...
    if (user.gmailTokens && viewportTxns.length > 0 && viewportTxns[0].emailId) {
      try {
        ...
        const emailId = viewportTxns[0].emailId;
