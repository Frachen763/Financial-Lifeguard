...
const checkGetHandsTransactions = async () => {
  try {
    console.log('🔍 Checking transactions to "Get Hands" merchant...');
    ...
    // Find all transactions to "Get Hands"
    const getHandsTxns = await Transaction.find({
      userId: user._id,
      merchant: { $regex: new RegExp('Get Hands', 'i') },
    }).sort({ transactionDate: -1 });
    
    console.log(`\n📊 Found ${getHandsTxns.length} transactions to "Get Hands":`);
    
    if (getHandsTxns.length === 0) {
      console.log('   No transactions found for "Get Hands" merchant');
      return;
    }
    
    getHandsTxns.forEach((txn, index) => {
    ...
    if (user.gmailTokens && getHandsTxns.length > 0 && getHandsTxns[0].emailId) {
      try {
        ...
        const emailId = getHandsTxns[0].emailId;
